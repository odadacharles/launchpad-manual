...
Launchpad has a large test suite. The normal pattern when developing is to
locally run the tests you think your change affects, and rely on post-merge
testing by `buildbot <hhttps://lpbuildbot.canonical.com/>`__ for full coverage.

There are two ways to run the tests:

- ``bin/test`` — the standard test runner. Start here.
- ``testr`` (testrepository) - an alternative wrapper you can use to iterate
  on failing tests.
...
.. tab-set::
   ...
   .. tab-item:: bin/test (recommended)
      ...
      You can also point the runner to a dotted module, which is the fastest
      way to iterate on a single module:
