...
permissions is how you create repositories. If you push to ``~jane/launchpad``,
Launchpad creates that repository under Jane's ownership; if you push to
``~my-team/launchpad``, it creates one owned by that team so every member of
...
- ``https://code.launchpad.net/~OWNER/PROJECT`` for an owner’s default
  repository for a forked upstream project.
- ``https://code.launchpad.net/~OWNER/DISTRIBUTION/+source/SOURCE`` for an
  owner’s default repository for a forked source package in a distribution. 
